...
function winnew(num, strm) {
    return strm.scale(() => win.scale[num])
...
// .layer(srcRelMask(s0)
// .scale(() => win.scale[0])
// .rotate(() => Math.sin(time*1.01+1) / 30)
// .scroll(() => win.x[0], () => win.y[0])
// //     .modulate(src(s1))
// )
.layer(srcRelMask(s0)
.scale(() => win.scale[1])
.rotate(() => Math.sin(time * 1.02 + 2) / 30)
...
.scroll(() => win.x[2], () => win.y[2]))

.layer(winnew(3, srcRelMask(s0)))
